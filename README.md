...
בפאנל, בתוך הגדרות הטופס של הכתבה, קובעים לאן הלידים שלה הולכים: **טאב משלה בגיליון** (ברירת מחדל: מזהה הקמפיין), **מייל התראה** ו-**webhooks** משלה. כל ליד נרשם תמיד גם בטאב הראשי "לידים", כך שלסוכנות יש תמונה מלאה ולכל לקוח טאב נקי משלו (עמודת "יעד" מציינת את הטאב). המייל וה-webhooks של הכתבה מתווספים ליעדים הכלליים. השרת קורא את הניתוב מ-`data/articles.json` (המטמון מתנקה בכל שמירת כתבה), וכפתור "ליד בדיקה לכתבה זו" בודק את היעדים של הכתבה כפי שנשמרו. כתבה שאינה מוכרת נרשמת רק בטאב הראשי, כדי שאי אפשר יהיה ליצור טאבים דרך הטופס הציבורי.
...
```json
{
  ...
  "clickId": "GiC...", "sheetTab": "top-ali", "flags": [], "test": false
}
```
